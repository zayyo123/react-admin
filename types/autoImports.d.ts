...
// 这个文件是一个自动生成的类型声明文件，主要作用包括：
// 自动导入声明：
// 定义了项目中所有可以自动导入的组件、函数和常量的类型声明
// 包括 UI 组件（如 BaseBtn、BaseTable 等）
// 包括工具函数（如 useClipboard、useEcharts 等）
// 包括配置常量（如 TOKEN、VERSION 等）
// 全局类型声明：
// 使用 declare global 声明全局可用的类型
// 为 TypeScript 提供类型检查和自动补全支持
// 类型重导出：
// 从其他类型文件中重新导出类型定义
// 包括表单相关类型、公共类型等
export {}
